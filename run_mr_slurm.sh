...
set -uo pipefail   # NB: no -e, so one failing config doesn't abort the whole grid

# ---- Environment ------------------------------------------------------------
# Activate the conda env built by setup_conda_env.sh (R + all packages +
# MR-PRESSO + PLINK). Change mr_env if you named it differently.
source "$(conda info --base)/etc/profile.d/conda.sh"
conda activate mr_env
export OMP_NUM_THREADS="${SLURM_CPUS_PER_TASK:-1}"   # data.table threads
...
# Common column mappings (kept as arrays so each run stays readable)
